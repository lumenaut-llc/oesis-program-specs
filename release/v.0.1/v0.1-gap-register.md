...
| --- | --- | --- | --- | --- |
| G1 | Named BOM vendors / SKU-level procurement (matrix already flags); also now includes reference instruments per `architecture/system/calibration-program.md` §A since reference-instrument files under `oesis-builds/procedures/<node>/references/` are de facto BOM items | Defer for lab v0.1; **Blocker** for naive third-party kitting | Release / hardware | `integrated-parcel-kit-bom.md` + checklist: add preferred suppliers or "bring your own" rules; reference-instrument rows per measurand per deployment class per `calibration-program.md` §A · Issue: [oesis-hardware#7](https://github.com/lumenaut-llc/oesis-hardware/issues/7) |
| G2 | Ingest authorization + strong parcel binding for live nodes | Defer | Technical | Design note + API hooks; matrix row already tracks · Issue: [oesis_runtime#6](https://github.com/lumenaut-llc/oesis_runtime/issues/6) |
| G3 | Continuous Wi-Fi (or other) transport from node to ingest with TLS, identity, retries | Defer | Technical | Transport lane now defined per deployment class in `architecture/system/deployment-maturity-ladder.md` "Transport tier by deployment class"; v0.1 assumes serial extract or HTTP demo with manual packet; v0.2 and later inherit class-appropriate transport from that table rather than re-deciding here · Issue: [oesis-program-specs#17](https://github.com/lumenaut-llc/oesis-program-specs/issues/17) |
| G4 | Real public weather/smoke feeds (keys, rate limits, fallbacks) | Defer | Technical | Operator doc for env vars / adapters when leaving fixtures · Issue: [oesis_runtime#7](https://github.com/lumenaut-llc/oesis_runtime/issues/7) |
...
| G7 | Automated diff between `oesis-program-specs/contracts/examples` and `oesis-runtime/oesis/assets/examples` | Defer | Technical | CI or release script per runtime README sync rule · Issue: [oesis-program-specs#18](https://github.com/lumenaut-llc/oesis-program-specs/issues/18) |
...
| G10 | PRD hazard claims vs bench-air-only sensing (flood, outage, freeze) | **Blocker** for marketing as full hazard suite; **Defer** for honest reference | Release + technical | Copy + scope matrix alignment; inference stays conservative · Issue: [oesis-program-specs#19](https://github.com/lumenaut-llc/oesis-program-specs/issues/19) |
...
| G15 | Admissibility rule in `architecture/system/calibration-program.md` §C is `planned`, not `implemented`; ingest pipeline cannot enforce the 8-point admissibility check, and §F build-spec front-matter metadata block is human-readable contract only — no automated validation against deployment-class standards in `deployment-maturity-ladder.md`; §G promotion-bar compliance relies on manual review | Partially addressed (program-phase `v0.2` / Milestone 2) | Technical | Two parts: (1) ingest adds `admissible_to_calibration_dataset: bool` + reason list on each normalized observation, wiring in all 8 §C checks against per-device calibration state and build-spec declarations; (2) front-matter validator reads §F block from `oesis-builds/specs/<node>/v0-X.md`, validates against `deployment-maturity-ladder.md` class standards, fails loudly on exceptions without justification. Both gate `v0.2` promotion via the 5th item in `../../architecture/current/pre-1.0-version-progression.md` promotion bar · **Step 1 (pure function + 28-test acceptance suite) shipped 2026-05-01 in [oesis_runtime#24](https://github.com/lumenaut-llc/oesis_runtime/pull/24)**; pipeline integration (step 2) and inference surfacing (step 3) remain. Front-matter validator (part 2) is **shipped** as `oesis-builds/tools/lint-specs.py` · Issue: [oesis_runtime#4](https://github.com/lumenaut-llc/oesis_runtime/issues/4) |
...
| G17 | Canonical observation schema in `oesis-contracts` lacks the facts that `architecture/system/calibration-program.md` §C admissibility check and `architecture/system/adapter-trust-program.md` §C admissibility check both depend on; without schema presence, every downstream consumer recomputes admissibility differently or the decision loses its audit trail | **Closed** 2026-05-01 | Technical (cross-repo) | Extended v1.0 observation schema in `oesis-contracts` with: `burn_in_complete`, `node_calibration_session_ref`, `node_deployment_maturity`, `node_deployment_class`, `protective_fixture_verified_at`, `placement_representativeness_class` (per calibration-program §C). All six fields optional and forward-compatible with v0.1 baseline. Decision 2026-04-19 (ADR 0009) honored: schema carries facts, runtime computes decision. Admissibility decision fields stay on normalized observations only · Shipped via [oesis-contracts#11](https://github.com/lumenaut-llc/oesis-contracts/pull/11) + runtime fixture mirror [oesis_runtime#20](https://github.com/lumenaut-llc/oesis_runtime/pull/20) · Closed [oesis-contracts#2](https://github.com/lumenaut-llc/oesis-contracts/issues/2) |
| G18 | Adapter-trust-program.md exists as policy for Tier 1 / Tier 2 adapter-derived evidence, but no adapter spec, source authority file, onboarding verification, or session log exists; `v1.5` bridge surfaces (equipment-state-adapter, indoor-response-node, passive thermal-slope inference) depend on this program being populated | Partially addressed (schema-side complete; broader program work deferred to capability stage `v1.5`) | Technical + hardware | **Schema-side shipped 2026-05-01** in [oesis-contracts#12](https://github.com/lumenaut-llc/oesis-contracts/pull/12) + runtime fixture mirror [oesis_runtime#22](https://github.com/lumenaut-llc/oesis_runtime/pull/22): v1.5 source-provenance-record now carries the five adapter-trust §C facts (`adapter_source_ref`, `adapter_contract_version`, `adapter_onboarding_ref`, `adapter_credential_last_verified_at`, `adapter_tier`), conditionally required when `source_kind == "adapter_derived"`. **Remaining (capability stage v1.5 work):** populate `oesis-builds/specs/adapters/<adapter>/v0-1.md` with §F front-matter, `oesis-builds/procedures/adapters/<adapter>/source.md` with §A source authority file, and per-parcel onboarding verification records. Runtime admissibility tooling from G15 step 1 already branches on `adapter_tier` to apply adapter-trust-program §C · Issue: [oesis-hardware#6](https://github.com/lumenaut-llc/oesis-hardware/issues/6) |
...
- **Closed:** G17 (2026-05-01)
- **Partially addressed:** G15 (step 1 / pure function shipped 2026-05-01; pipeline integration + inference surfacing remain), G18 (schema-side shipped 2026-05-01; broader v1.5 program work remains)
- **Blocker for v0.2 promotion:** G11, G12, G13, G14, G16, G23 (G17 was here, now closed)
- **Blocker for v1 formula fitting** (subset of above): G13, G14, G15
- **Blocker for external pilot** (regardless of slice): G19
- **Defer (v1.5 or later):** G18 (broader work), G22
- **PRD-only:** G5, G6, G24
