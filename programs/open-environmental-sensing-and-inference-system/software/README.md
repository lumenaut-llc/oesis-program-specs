...
- `../../../oesis/` is the canonical Python implementation tree for the current MVP services.
- `scripts/rhi_*.sh` remains as a legacy command compatibility layer during the migration.
- `programs/open-environmental-sensing-and-inference-system/software/*/scripts/` stays in place as a compatibility layer for docs, runbooks, and existing operator commands.
- New implementation work should land in `../../../oesis/` first, then flow through the docs-facing script entrypoints.
...
- keep implementation truth in `../../../oesis/` rather than creating competing
