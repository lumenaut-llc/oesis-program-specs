...
- `programs/open-environmental-sensing-and-inference-system/hardware/bench-air-node/operator-runbook.md`
- `programs/open-environmental-sensing-and-inference-system/hardware/mast-lite/operator-runbook.md`
...
curl -s -X POST http://127.0.0.1:8787/v1/ingest/node-packets \
  ...
  --data-binary @programs/open-environmental-sensing-and-inference-system/docs/data-model/examples/node-observation.example.json
```
...
The public preview site now lives as an Astro app in a stable top-level site
directory.

From:

```bash
cd sites/public-preview
```
