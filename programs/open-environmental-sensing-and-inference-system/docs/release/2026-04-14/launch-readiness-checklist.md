...
- Operator dry run on April 1, 2026 succeeded for `python3 -m oesis.parcel_platform.reference_pipeline`, `python3 -m oesis.ingest.ingest_packet programs/open-environmental-sensing-and-inference-system/docs/data-model/examples/node-observation.example.json --parcel-id parcel_demo_001`, and preview-site HTTP checks returning `200 OK` for `http://127.0.0.1:8000/`.
