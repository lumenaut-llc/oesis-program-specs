...
- `repo/rhi/` is the canonical Python implementation tree for the current reference services.
...
| --- | --- | --- |
| `rhi.bench-air.v1` | `air.node.snapshot` | implemented |
| `rhi.bench-air.v1` from `mast-lite` | `air.node.snapshot` with outdoor install metadata | partially implemented through shared lineage |
| `rhi.weather-pm-mast.v1` | `air.pm_weather.snapshot` | not yet implemented |
| `rhi.flood-node.v1` | `flood.low_point.snapshot` | not yet implemented |
| `rhi.thermal-pod.v1` | `thermal.scene.snapshot` | not yet implemented |
...
- Use `repo/rhi/` as the canonical Python implementation tree.
- Keep `programs/resilient-home-intelligence/software/*/scripts/` as compatibility entrypoints only.
