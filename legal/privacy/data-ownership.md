...
Private by default.
Shared by choice.
Parcel operators should own the raw data generated by their parcel deployments.

## What the parcel operator owns

The project’s working rule is that the parcel operator owns and controls raw parcel-linked observations generated by sensors or devices they deploy on their property or for their household use.

This includes, where applicable:

- raw sensor observations
- parcel-linked device data
- exact parcel-linked histories associated with that parcel operator account
- parcel metadata or notes supplied by the parcel operator
...
- receive participant-contributed data
- store it for product operation
- normalize and process it
- generate parcel-state outputs for the parcel operator
- apply the sharing settings the parcel operator selected
...
participant-contributed private parcel-linked data into public data.
...
- use private parcel data to provide the parcel operator’s parcel view and related platform operation
...
- infer that device compatibility or control capability implies parcel approval to actuate it

## Derived outputs

Derived parcel-state outputs are platform-generated interpretations about the parcel operator’s parcel.

Those outputs do not erase the parcel operator’s rights in the underlying raw data.

The project’s current policy direction is:

- the parcel operator should be able to view current parcel-state outputs tied to their parcel
...
The working rule is that parcel stewardship should be supported by practical controls, not branding language alone.

The project therefore intends to support:

- export of participant-contributed observations and key parcel-state history
